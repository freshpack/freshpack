...
const logVersionWarning = (str) => {
  ...
  if (latestPackageVersion && latestPackageVersion !== pkg.version) {
    ...
    // console.log(spacer + colors.magenta('Please update to v' + latestPackageVersion + ' !'));
...
const writeFile = (filePath, content) => {
  ...
  fs.writeFile('./' + projectDirName + '/' + filePath, content, (err) => {
    ...
    const msg = '▪︎ '.white + filePath + fileName;
    log(msg);
    return next();
  });
};
...
const exit = () => {
  ...
  if (!cmdLineArgs.quiet) {
    ...
    displayDependencies(projectDirName);
    log('');
  ...
  if (cmdLineArgs.lint) {
    log(colors.white('cd ' + projectDirName + ' && yarn lint'));
  }

  if (cmdLineArgs.test) {
    log(colors.white('cd ' + projectDirName + ' && yarn test'));
  }

  log(colors.white('cd ' + projectDirName + ' && yarn start'));
